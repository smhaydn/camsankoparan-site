    ...
    <main className="mx-auto max-w-7xl px-3 py-6 sm:px-6">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink/50">Şantiye İş Programı</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="hidden text-sm text-ink/50 sm:inline">👷 {kullaniciAdi}</span>
          <button
            onClick={yeniEkle}
          ...
          <button
            ...
            className="rounded-full border border-black/15 px-4 py-2 text-sm text-ink/80 transition hover:border-bronze hover:text-bronze"
          >
            Geçmiş
          </button>
          <button
            onClick={cikis}
            className="rounded-full border border-black/15 px-4 py-2 text-sm text-ink/60 transition hover:border-red-400 hover:text-red-400"
          >
            Çıkış
          </button>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <input
          ...
          className="min-w-[180px] flex-1 rounded-md border border-black/15 bg-black/5 px-3 py-2 text-sm outline-none focus:border-bronze"
        />
        <select
          value={blokF}
          onChange={(e) => setBlokF(e.target.value)}
          className="rounded-md border border-black/15 bg-white px-3 py-2 text-sm outline-none focus:border-bronze [color-scheme:light] [&>option]:bg-white [&>option]:text-ink"
        >
        ...
        <div className="flex overflow-hidden rounded-full border border-black/15">
          <button
            ...
            className={`px-4 py-2 text-sm ${gorunum === "tablo" ? "bg-bronze text-ink" : "text-ink/60"}`}
          >
            Tablo
          </button>
          <button
            onClick={() => setGorunum("gantt")}
            className={`px-4 py-2 text-sm ${gorunum === "gantt" ? "bg-bronze text-ink" : "text-ink/60"}`}
          >
            Zaman Çizelgesi
          </button>
        </div>
...
function Ozet({ label, value, renk }: { label: string; value: number | string; renk?: string }) {
  return (
    <div className="rounded-lg border border-black/10 bg-black/[0.03] p-3 text-center">
      <div className="font-display text-2xl font-bold" style={{ color: renk ?? "#c08a4d" }}>
        {value}
      </div>
      <div className="mt-0.5 text-[11px] tracking-wide text-ink/50 uppercase">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-black/10">
      <table className="w-full min-w-[900px] text-sm">
        <thead>
          <tr className="border-b border-black/10 bg-black/[0.03] text-left text-xs text-ink/50">
            ...
            return (
              <tr key={i.id} className="border-b border-black/5 hover:bg-black/[0.02]">
                <td className="px-3 py-2.5">
                  <div className="font-medium text-ink/90">{i.blok || "—"}</div>
                  <div className="text-xs text-ink/45">{i.kat || ""}</div>
                </td>
                <td className="px-3 py-2.5">
                  <div className="text-ink/90">{i.aciklama1 || i.imalat || "—"}</div>
                ...
                <td className="px-3 py-2.5 whitespace-nowrap text-ink/70">
                  {trTarih(i.baslangic)} <span className="text-ink/30">→</span> {trTarih(i.bitis)}
                </td>
                <td className="px-3 py-2.5 text-center text-ink/70">{i.gun ?? "—"}</td>
                <td className="px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-ink/60">%{yuzde}</span>
                  </div>
                </td>
                <td className="px-3 py-2.5">
                  <select
                    ...
                    className="rounded-md border border-black/10 bg-white px-2 py-1 text-xs outline-none focus:border-bronze [color-scheme:light] [&>option]:bg-white [&>option]:text-ink"
                ...
                <td className="px-3 py-2.5 whitespace-nowrap text-ink/70">{i.ekip || "—"}</td>
                <td className="px-3 py-2.5 whitespace-nowrap text-right">
                  <button
                    onClick={() => onDuzenle(i)}
                    className="rounded px-2 py-1 text-xs text-ink/60 transition hover:text-bronze"
                  >
                    Düzenle
                  </button>
                  <button
                    onClick={() => onSil(i)}
                    className="rounded px-2 py-1 text-xs text-ink/30 transition hover:text-red-400"
                  >
                    Sil
                  </button>
                </td>
