...
export function Gecmis({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-white/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="h-full w-full max-w-md overflow-y-auto border-l border-black/15 bg-white p-6 shadow-2xl"
      ...
      >
        ...
        {!log && !err && <p className="text-sm text-ink/40">Yükleniyor…</p>}
        {log && log.length === 0 && <p className="text-sm text-ink/40">Henüz kayıt yok.</p>}
        ...
        <div className="flex flex-col gap-3">
          {log?.map((k) => {
            ...
            return (
              <div key={k.id} className="rounded-lg border border-black/10 bg-black/[0.03] p-3">
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span className="font-semibold text-ink/90">{k.kullanici || "—"}</span>
                  <span
                  ...
                  <span className="text-ink/40">{tarih}</span>
                </div>
                <div className="mt-1 text-sm text-ink/70">{k.is_ozet || "—"}</div>
                {k.degisiklikler && Object.keys(k.degisiklikler).length > 0 && (
                  <ul className="mt-2 space-y-0.5 border-t border-black/5 pt-2 text-xs text-ink/50">
                    {Object.entries(k.degisiklikler).map(([alan, d]) => (
                      <li key={alan}>
                        <span className="text-ink/60">{ALAN_ETIKET[alan] ?? alan}:</span>{" "}
